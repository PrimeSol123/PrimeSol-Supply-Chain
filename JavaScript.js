...
function renderTable() {
    ...
    orderData.forEach((item, index) => {
        ...
        tr.innerHTML = `
            ...
            <td>${item.weight}</td>
            <td>${item.country}</td>
            <td><span class="detail-btn" data-field="receiver" data-index="${index}">${cutStr(item.receiver)}</span></td>
            <td>${item.declareValue}</td>
            <td>${item.transport}</td>
            <td>${item.supplyChain}</td>
            <td>${item.channel}</td>
            <td>${item.warehouseOut}</td>
            <td><span class="detail-btn" data-field="payable" data-index="${index}">${cutStr(item.payable.toFixed(2))}</span></td>
            <td><span class="detail-btn" data-field="costRemark" data-index="${index}">${cutStr(item.costRemark)}</span></td>
            <td>${item.customerPay.toFixed(2)}</td>
            <td><span class="detail-btn" data-field="quoteDetail" data-index="${index}">${cutStr(item.quoteDetail)}</span></td>
            <td>${item.profit.toFixed(2)}</td>
            <td>${item.trackingNo}</td>
            <td><span class="detail-btn" data-field="trackingStatus" data-index="${index}">${cutStr(item.trackingStatus)}</span></td>
            <td>${item.customs}</td>
            <td><span class="detail-btn" data-field="remark" data-index="${index}">${cutStr(item.remark)}</span></td>
...
function bindAllEvents() {
    ...
    document.getElementById('add-pay-item').onclick = () => {
        const amount = parseFloat(document.getElementById('form-payable').value) || 0;
        const remark = document.getElementById('form-cost-remark').value.trim();
        if (amount <= 0 || !remark) {
            alert('请填写有效的金额和备注');
            return;
        }
    ...
    document.getElementById('add-tracking-record').onclick = () => {
        const time = document.getElementById('form-tracking-status').value;
        const content = document.getElementById('form-remark').value.trim();
        if (!time || !content) {
            alert('请填写轨迹时间和内容');
            return;
        }
...
function renderTempPayItems() {
    ...
    if (tempPayItems.length === 0) {
        container.innerHTML = '<p style="color: #999; margin: 8px 0;">暂无应付子项，点击"新增应付子项"添加</p>';
        return;
    }
    tempPayItems.forEach((item, index) => {
        const itemEl = document.createElement('div');
        itemEl.style.cssText = 'display: flex; align-items: center; gap: 12px; margin: 8px 0; padding: 8px; background: #f5f7fa; border-radius: 4px;';
        itemEl.innerHTML = `
            <span>¥${item.amount.toFixed(2)}</span>
            <span style="flex: 1;">${item.remark}</span>
            <button onclick="removePayItem(${index})" style="color: #ff6b6b; border: none; background: none; cursor: pointer;">×</button>
        `;
...
function renderTempTrackingRecords() {
    ...
    if (tempTrackingRecords.length === 0) {
        container.innerHTML = '<p style="color: #999; margin: 8px 0;">暂无轨迹记录，点击"新增轨迹记录"添加</p>';
        return;
    }
    tempTrackingRecords.forEach((item, index) => {
        ...
        itemEl.innerHTML = `
            ...
            <button onclick="removeTrackingRecord(${index})" style="color: #ff6b6b; border: none; background: none; cursor: pointer;">×</button>
        `;
        container.appendChild(itemEl);
    });
}
...
// 移除应付子项（全局函数，供弹窗内调用）
window.removePayItem = (index) => {
    tempPayItems.splice(index, 1);
    renderTempPayItems();
};

// 移除轨迹记录（全局函数，供弹窗内调用）
window.removeTrackingRecord = (index) => {
    tempTrackingRecords.splice(index, 1);
    renderTempTrackingRecords();
};
